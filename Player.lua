...
function Player:update(dt)
	...
	if self.collider:enter('Enemy') then
		...
		print("Player: " .. self.life)
	end
end

function Player:draw()
	love.graphics.draw(self.hero,self.quad,self.pos.x-18,self.pos.y-50,math.rad(self.angle),self.scale.x * self.flip.x,self.scale.y * self.flip.y,self.w/6,self.h/6) -- yay! we are drawing the hero
